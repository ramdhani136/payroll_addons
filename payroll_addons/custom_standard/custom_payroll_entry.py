...
def create_salary_slips_custom(self):
	...
	for row in self.employees:
		if row.employee:
			if frappe.get_doc("Employee",row.employee).grade:
				row.employee_grade = frappe.get_doc("Employee",row.employee).grade
				row.db_update()
...
def create_salary_slips_for_employees_custom(employees, args, publish_progress=True):
	...
	for emp in employees:
		if emp not in salary_slips_exists_for:
			...
			# ss.work_period = 0
			# ss.forget_to_checkout = 0
			# ss.paid_leave = 0
			# ss.late_count = 0

			if len(get_attendance) > 0:
				args.update({
					"work_period" : get_attendance[0][0],
					"forget_to_checkout" : get_attendance[0][1],
					"paid_leave" : get_attendance[0][2],
					"late_count" : get_attendance[0][3]
				})
